...
allows one to find very faint objects.  Microlensing searches of exoplanets in M31 
...
recognized in advance \citep{griest98,gould10}.  Median Einstein timescale of 
...
to efficiently find high mass-ratio planets.  
...
sampling is enough to discover microlensing events and most of planet 
sensitivity comes from the follow-up observations.  
If most of planet sensitivity comes from the LSST observations, 
then cadence would need to be improved to $1~{\rm hour}$.  
}
...
The depths may be relaxed at the begin and end of SMC observing season 
in order to extend time interval each year.  

SMC is located close to South ecliptic pole, hence can be observed independent of Moon phase.
...
High detection efficiency for microlensing events requires uniform sampling and
...
The detection of microlensing events is best done with uniform sampling.  
We request higher cadence at the beginning of the survey 
(until $>15$ epochs are collected in $r$, $g$, and $i$ bands), 
because microlensing events can be found only after it is found, which stars are constant.  
In order to achieve uniform cadence, 
...
The number of the monitored sources is the highest in $r$ band.  
Slightly smaller number is in $g$ band, and 
...
The total exposure time per visit is constrained by desired depth of a single visit.  
Saturation of brightest objects is not an issue because there are almost 
no events with sources saturated in standard LSST visits and if an event 
becomes brighter than saturation limit, then follow-up resources 
will provide adequate photometric coverage.
...
that is observed with standard Wide-Fast-Deep cadence in {\tt baseline\_2018a} 
...
the SMC center, then high cadence in 2 or 3 fields would be required.  
...
Uniform cadence is requested and is most advantageous for proposed science program.  
The increase of visit frequency (at the cost of lower frequency at other times) 
will only help if short-lasting caustic-crossing features are predicted, but in 
those cases follow-up resources should be used.
...
events to be observed, but their value will be lower due to limiting depth of 
...
Only a single field is proposed here and uniform sampling in time is more 
...
We expect that there will be other proposals that request observations of SMC and LMC 
\citep[see Sec.\, 7 in][]{marshall17}.  Most of the other proposals will probably 
put stronger requests on observing a large are of the Clouds at the expense of lower cadence.  
Our proposal is most beneficial at the center of SMC.  We expect variability studies 
in SMC (including planetary transits) will be optimized towards the largest number of stars monitored, 
which is in accordance with plan proposed here. Observations in $u$, $z$, and $y$ bands may be requested 
in other proposals, but these bands have low value for science program proposed here.  
If they are executed in addition to our program, then they will improve source characterization 
to small degree. 
