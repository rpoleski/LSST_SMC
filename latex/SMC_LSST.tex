...
\author{
Rados\l{}aw Poleski\footnote{Department of Astronomy, Ohio State University, 140 W. 18th Ave., Columbus, OH 43210, USA} and 
Przemek Mr\'oz\footnote{Warsaw University Observatory, Al. Ujazdowskie 4, 00-478 Warszawa, Poland}}
\date{November 2018}
...
the planet formation and evolution in different environments.  We discuss what changes in 
...
larger distances.  We propose high cadence observations of 
the Small Magellanic Cloud (SMC) by LSST, and show that these observations will allow 
...
For the SMC events, in most cases even the light from the planet host will not be detectable.  
...
the SMC are almost exclusively of self-lensing type, i.e., both the lens and 
...
a short-lasting anomaly \citep{mao91,gould92} on top of 
a point-source point-lens event \citep{paczynski86}.  The length of 
the anomaly is on the order of the Einstein timescale of the host event 
...
the SMC events is $0.35~M_{\odot}$ and 
...
when compared to the point-source point-lens model.  
...
(maximum magnification $\approx 80$) and 
the planet with $s=1.04$, $q=7.9\times10^{-4}$ 
is detected with $\Delta\chi^2 = 2900$ in proposed strategy.  
The event timescale is $89~{\rm d}$ and it is detected at ${\rm JD} = 2463037$.
...
large part of region of interest in just two pointings. 
...
by microlensing.  Below we present expected yield that assumes a fiducial 
planet frequency function and request the experiment with yield of a few planets 
so that, scientifically interesting upper limits on 
the planet frequency can be derived even if no planets are detected.
...
If most of the planet sensitivity comes from the LSST observations, 
then the cadence would need to be improved to 1 hour.  
}

We propose for on the order of 2000 visits
to be taken in each of two fields in the center of the SMC.  
...
If only a single pointing is chosen, then it should be placed at
...
We request $r$ and $g$ band image depths of $\gtrsim23.5~{\rm mag}$ 
and $i$ band depth of $\gtrsim23.0~{\rm mag}$.  
...
The optimal strategy is to take one or two visits per night.  
...
If a single field is observed, then these numbers are 
...
which is a number of the SMC planets discovered.  The number is evaluated 
...
We assume that the follow-up observations will be conducted 
using a $\approx4$-${\rm m}$ telescope and the same exposure time as for LSST.  
...
in a few fields close to the SMC), see \citet{ivezic18}. 
...
We may expect that the planet frequency is lower in the SMC, but for 
calculating the FoM, we assume the same as in the Galactic bulge 
and request sampling that leads to at least a few planets are discovered under this assumption. 
...
we multiply event rate in the sky-area considered ($2.7\times10^{-7}~{\rm yr^{-1}}$), 
...
In {\tt baseline\_2018a} anomalies are detected for only $0.0017$ of events and 
expected planet yield is $0.50$. 
